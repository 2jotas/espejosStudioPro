...
export const LOOK_OPTIONS: LookOption[] = [
  {
    ...
    icon: '📷',
    cssFilter: 'none',
    description: 'Foto real directa de cámara sin retoques ni filtros.',
  ...
  },
  {
    ...
    badge: '✨ Estudio',
    icon: '✨',
    cssFilter: 'contrast(1.06) brightness(1.03) saturate(0.96)',
    description: 'Luz de estudio limpia y nítida. Resalta textura del corte y mantiene tonos naturales.',
  ...
  },
  {
    ...
    badge: '🌟 Campaña',
    icon: '🌟',
    cssFilter: 'contrast(1.10) brightness(1.00) saturate(0.98) sepia(0.04)',
    description: 'Estilo editorial de alta gama. Negros profundos, profundidad y micro-viñeta.',
    ...
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            disabled={isUploading}
            className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-lg shadow-indigo-600/25 transition-all flex items-center space-x-2 cursor-pointer"
          >
            {isUploading ? (
              <>
      ...
      {modalItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/85 backdrop-blur-md overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md sm:max-w-lg w-full p-4 sm:p-5 space-y-3.5 shadow-2xl relative my-auto max-h-[92vh] flex flex-col justify-between overflow-y-auto">
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center space-x-2">
                <Sparkles className="w-4 h-4 text-indigo-400" />
                <h3 className="text-sm sm:text-base font-bold text-white">Estudio de Looks Espejos</h3>
              </div>
              <button
            ...
            {/* 3 Look Presets Chips (Instant CSS Preview) */}
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                  Elige un acabado:
                </span>
                <span className="text-[10px] text-slate-500">Preview instantáneo en vivo</span>
              </div>
                ...
                {LOOK_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-2 px-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex flex-col items-center space-y-0.5 relative border ${
                        isSelected
                    ...
                    >
                      ...
                      {isCurrentBaked && (
                        <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-emerald-500 border border-slate-900 rounded-full" title="Look actual guardado" />
                      )}
                    </button>
                  );
                })}
              ...
              <p className="text-[10px] sm:text-[11px] text-slate-400 mt-1.5 italic text-center">
            ...
            <div className="flex flex-col items-center justify-center py-0.5">
              <div className="w-48 sm:w-60 max-h-[42vh] aspect-[4/5] rounded-2xl overflow-hidden bg-slate-950 border-2 border-[#8B7CFF] shadow-[0_0_25px_rgba(139,124,255,0.3)] relative group mx-auto">
                ...
                <div className="absolute bottom-2 left-1/2 -translate-x-1/2 px-2.5 py-0.5 bg-slate-950/80 backdrop-blur-md rounded-full border border-white/10 text-[9px] font-mono text-slate-300 z-20 whitespace-nowrap shadow-lg">
                  {formatShortDate(modalItem.publishedAt || modalItem.createdAt)}
                </div>

                {/* Watermark/Status Indicator */}
                <div className="absolute top-2 right-2 px-2 py-0.5 rounded-md bg-slate-950/80 backdrop-blur-md text-[9px] font-bold uppercase tracking-wider text-slate-200 border border-white/10 z-20 flex items-center space-x-1">
              ...
              {modalItem.rawUrl && (
                <button
                  ...
                  className="mt-2.5 text-[11px] text-slate-400 hover:text-indigo-300 font-medium cursor-pointer select-none bg-slate-800/70 px-3 py-1 rounded-xl border border-slate-700/60 transition-colors"
                >
            ...
            <div className="flex items-center justify-between border-t border-slate-800 pt-3">
              <div className="text-[11px] text-slate-400">
                {modalItem.appliedLook === selectedPreviewLook ? (
                  <span className="text-emerald-400 font-semibold flex items-center gap-1">
                    <Check className="w-3 h-3" /> Look ya aplicado
                  </span>
                ) : (
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className="px-3.5 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-indigo-600/25 transition-all flex items-center space-x-1.5 cursor-pointer disabled:opacity-50"
                >
